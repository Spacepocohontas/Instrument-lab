(()=>{
  ...
  const keyForMidi=n=>{
    const delta=n-60;
    if(delta>=0&&delta<13)return keys[delta];
    return null;
  };
  ...
  async function connectMIDI(){
    ...
    try{
      ...
      midi.onstatechange=()=>{
        for(const input of midi.inputs.values())input.onmidimessage=midiMessage;
        setStatus(`MIDI ready · ${midi.inputs.size} input${midi.inputs.size===1?'':'s'}`)
      };
      setStatus(count?`MIDI ready · ${count} input${count===1?'':'s'}`:'MIDI ready · connect a controller');
  ...
  addEventListener('beforeunload',()=>pressed.forEach((key,note)=>sendKey('keyup',key)));
})();
